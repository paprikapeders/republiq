            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                    ...
                    {filteredTeams.map((team, index) => {
                        ...
                        return (
                            <Link
                                key={team.id}
                                href={`/teams/${team.id}`}
                                className="block bg-[#1a1a2e] border border-[#16213e] hover:border-orange-500/30 transition-all overflow-hidden group rounded-lg"
                            >
                                ...
                                <div className="p-6">
                                    <div className="flex items-center justify-between">
                                        ...
                                        {team.leagues?.[0] && (
                                            <div className="text-xs text-gray-500 bg-[#0f0f1e] px-2 py-1 rounded">
                                                {team.leagues[0].name}
                                            </div>
                                        )}
                                    </div>
                ...
                {filteredTeams.length === 0 && (
                    <div className="bg-[#1a1a2e] border border-[#16213e] p-12 text-center rounded-lg">
                        <p className="text-gray-400">No teams registered for this season yet.</p>
                    </div>
                )}
            </div>
        </div>
